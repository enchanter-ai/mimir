...
| **G2** Adversarial attack resistance | **PROVED** | `python verify-all.py` in `spec/test-vectors-adversarial/` — 12/12 vectors, 11 correctly REJECT (sig truncate/flip/zero, request/result digest tamper, tool_call_id swap, replay-window past/future, alg downgrade, key_id swap, extra-unknown-field), 1 correctly VERIFY (whitespace-added — proves canonicalizer is whitespace-stable) | Larger attack surface (DPoP forgery, source-binding bypass, partial-result injection) once those features are implemented |
...
| On-chain anchor layer + EigenLayer slashing | narrative only | **Anchor + slashing wired; 12/12 simulated-EVM tests pass (7 permissionless + 5 AVS slashing)** |
...
# Adversarial vectors
cd spec/test-vectors-adversarial && python verify-all.py    # 12/12 PASS
...
cd anchor/go && CGO_ENABLED=0 go test -v ./...      # 12/12 PASS (7 permissionless + 5 AVS)
...
- **7/7 are now demonstrably closed** at the protocol + service-correctness layer:
  ...
  - G2 Adversarial resistance — 12/12 attack vectors handled correctly
  - G3 Key custody (AWS KMS) — wire-faithful fake routes through HTTP; sig verifies externally
  - G4 On-chain anchor + EigenLayer slashing — 12/12 simulated-EVM tests (7 permissionless + 5 AVS slashing); operator gating, fraud-proof slashing, and multi-operator isolation all proven against `IServiceManager` + `ISlasher` interfaces
